...
time_intervals2 = {'2015_2020','2021_2030','2031_2040','2041_2050',     ...
...
for iCf = 5 : length(Climate_forcing)
    ...
    for i = 1 : length(scenarios)
        for j = 1 : length(vars)
            if ~exist(['./'  Climate_forcing{iCf} '/' scenarios{i} '/'  ...
                       vars{j}],'dir')
                mkdir(['./'  Climate_forcing{iCf} '/' scenarios{i} '/'  ...
                       vars{j}]);
            end
            ...
            for k = 1 : length(time_intervals)
                url = [str1 scenarios{i} '/' vars{j} str2 prefix        ...
                       scenarios{i} '_' vars{j} suffix time_intervals{k}...
                       '.nc'];
                filename = ['./'  Climate_forcing{iCf} '/' scenarios{i} ...
                            '/' vars{j} '/' prefix scenarios{i} '_'     ...
                            vars{j} suffix time_intervals{k} '.nc'];
                if ~exist(filename,'file')
                    ...
                    try 
                        ...
                        disp([filename ' is not correctly downloaded!']);
                        disp('Re-downloading now:');
                        outfilename = websave(filename,url,options);
                    end

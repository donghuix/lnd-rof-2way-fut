...
if strcmp(domain,'NLDAS')
    if exist('sunrise_set.mat','file')
        load('sunrise_set.mat');
    else
    [m,n] = size(ele);
    hr_min = NaN(m,n,12,31);
    hr_max = NaN(m,n,12,31);
    longxy = ncread('/project/projectdirs/m3780/donghui/Runoff_Projection_Uncertainty/inputdata/domain.lnd.NLDAS_0.5x0.5_isimip.3b.c211109.nc','longxy');
    latixy = ncread('/project/projectdirs/m3780/donghui/Runoff_Projection_Uncertainty/inputdata/domain.lnd.NLDAS_0.5x0.5_isimip.3b.c211109.nc','latixy');
    for i2 = 1 : m
        for j2 = 1 : n
            for im = 1 : 12
    ...
    for j = 1 : 2
        ...
        parfor k = 1 : length(time_intervals)
            ...
            for ivar = 1 : length(varread)
                filename = [upper(model) '/' scenarios{i} '/' varread{ivar} '/' model '_r1i1p1f2_w5e5_' scenarios{i} '_' varread{ivar} '_global_daily_' time_intervals{k} '.nc'];
                if strcmp(domain,'NLDAS')
...
for i = 1 : length(scenarios)
    ...
    parfor k = 1 : length(time_intervals)
        ...
        filename1 = [upper(model) '/' scenarios{i} '/psAdjust/'      model '_r1i1p1f2_w5e5_' scenarios{i} '_psAdjust_global_daily_'      time_intervals{k} '.nc'];
        filename2 = [upper(model) '/' scenarios{i} '/sfcWindAdjust/' model '_r1i1p1f2_w5e5_' scenarios{i} '_sfcWindAdjust_global_daily_' time_intervals{k} '.nc'];
        filename3 = [upper(model) '/' scenarios{i} '/hussAdjust/'    model '_r1i1p1f2_w5e5_' scenarios{i} '_hussAdjust_global_daily_'    time_intervals{k} '.nc'];
        filename4 = [upper(model) '/' scenarios{i} '/tasmaxAdjust/'  model '_r1i1p1f2_w5e5_' scenarios{i} '_tasmaxAdjust_global_daily_'  time_intervals{k} '.nc'];
        filename5 = [upper(model) '/' scenarios{i} '/tasminAdjust/'  model '_r1i1p1f2_w5e5_' scenarios{i} '_tasminAdjust_global_daily_'  time_intervals{k} '.nc'];
